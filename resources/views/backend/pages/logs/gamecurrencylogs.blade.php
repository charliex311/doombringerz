                    ...
                    <div class="card-inner">
                        <div class="card-title-group">
                            <h5 class="card-title" style="display: flex;">
                                ...
                                <a href="{{ route('logs.gamecurrencylogs') }}?period=day&date={{ date('Y-m-d') }}" class="btn btn-sm btn-primary" style="margin-right: 15px;">
                                    <em class="icon ni ni-calender-date mr-sm-1"></em>
                                    <span class="d-none d-sm-inline">{{ __('Сегодня') }}</span>
                                </a>
                                <a href="{{ route('logs.gamecurrencylogs') }}?period=week" class="btn btn-sm btn-primary">
                                    <em class="icon ni ni-calender-date mr-sm-1"></em>
    ...
    <script>
        $(document).ready(function(){
            $('#date').on('change', function() {
                document.location.replace('{{ route('logs.gamecurrencylogs') }}?period=day&date='+$('#date').val());
            });
        });
    </script>
@endpush