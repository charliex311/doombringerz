...
@section('headerDesc', __('Настройки') . ' ' . __('О нас'))
                                        ...
                                        <div class="col-lg-12">
                                            <div class="form-group">
                                                <label class="form-label"
                                                       for="about_description_{{ $key }}">{{ __('Описание') }}</label>
                                                <div class="form-control-wrap">
                                                     <textarea type="text" class="form-control"
                                                         id="about_description_{{ $key }}"
                                                         name="setting_about_description_{{ $key }}">{{ config('options.about_description_'.$key) }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
@push('scripts')
    <script>
        @foreach(getlangs() as $key => $value)
            ...
            CKEDITOR.replace( 'about_description_{{ $key }}', {
