...
@section('headerDesc', __('Настройки') . ' ' . __('Частые вопросы'))
                                        ...
                                        <div class="col-lg-12">
                                            <div class="form-group">
                                                <label class="form-label"
                                                       for="faq_description_{{ $key }}">{{ __('Описание') }}</label>
                                                <div class="form-control-wrap">
                                                     <textarea type="text" class="form-control"
                                                         id="faq_description_{{ $key }}"
                                                         name="setting_faq_description_{{ $key }}">{{ config('options.faq_description_'.$key) }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
@push('scripts')
    <script>
        @foreach(getlangs() as $key => $value)
            ...
            CKEDITOR.replace( 'faq_description_{{ $key }}', {
