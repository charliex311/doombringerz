...
@section('wrap')
                            ...
                            <li>
                                <div class="drodown">
                                    <a href="#" class="dropdown-toggle btn btn-white btn-dim btn-outline-light" data-toggle="dropdown">
                                        ...
                                        <span>Фильтр{{ request()->has('class') ? (': ' . __(request()->query('class'))) : '' }}</span>
                                        <em class="dd-indc icon ni ni-chevron-right"></em>
                                    </a>
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <ul class="link-list-opt no-bdr">
                                            <li><a href="{{ route('auction.index') }}"><span>Все</span></a></li>
                                            <li><a href="{{ route('auction.index', ['class' => 'weapon']) }}"><span>Оружие</span></a></li>
                                            <li><a href="{{ route('auction.index', ['class' => 'armor']) }}"><span>Броня</span></a></li>
                                            <li><a href="{{ route('auction.index', ['class' => 'accessary']) }}"><span>Аксессуары</span></a></li>
                                            <li><a href="{{ route('auction.index', ['class' => 'etc']) }}"><span>Разное</span></a></li>
                                        </ul>
                                    </div>
                                </div>
                            </li>
    ...
    <div class="nk-block">
        <div class="row g-gs">
                        ...
                        <div class="card-inner">
                            <div class="project">
                                ...
                                <div class="project-details">
                                    <p class="m-0">Текущая ставка: <span class="fw-bold text-primary">{{ $lot->current_bet ?? $lot->start_price }} <em class="icon ni ni-coins"></em></span></p>
                                    <p class="m-0">Завершится <span class="fw-bold text-primary">{{ $lot->end_at->diffForHumans() }}</span></p>
                                </div>
                                <div class="project-progress">
                                    <div class="project-progress-details">
                                        <div class="project-progress-task"><em class="icon ni ni-check-round-cut"></em><span>{{ $lot->count }} {{ plural_form($lot->count, ['ставка', 'ставки', 'ставок']) }}</span></div>
                                        <div class="project-progress-percent">Блиц: <span class="fw-bold text-primary">{{ $lot->buyout_price }} <em class="icon ni ni-coins"></em></span></div>
                                    </div>
            ...
            @empty
                <p class="ml-3 mt-3 text-gray">Нет доступных лотов.</p>
            @endforelse
        </div>
        <div class="card-inner">
